Feature: "net_tester run" コマンド
  ...
  Scenario: ホスト数 <= 0 だとエラー
    When コマンド `net_tester run --device eth1 --nhost 0 --dpid 0x123` を実行
    Then 終了ステータスは 0 ではない
    And コマンドの出力は "--nhost must be > 0" を含む

  Scenario: 2 回実行するとエラー
    Given コマンド `net_tester run --device eth1 --nhost 3 --dpid 0x123` の実行に成功
    When コマンド `net_tester run --device eth1 --nhost 3 --dpid 0x123` を実行
    Then 終了ステータスは 0 ではない
