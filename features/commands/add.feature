Feature: net_tester add コマンド
  ...
  Scenario: パッチを追加して送受信に成功
    When コマンド `net_tester add --vport 1 --port 1 -S sockets` を実行
    Then 終了ステータスは 0
    And コマンドの出力はなし
    When コマンド `net_tester add --vport 2 --port 2 -S sockets` を実行
    Then 終了ステータスは 0
    And コマンドの出力はなし
  ...
  Scenario: --port に大きすぎるポート番号を指定するとエラー
    When コマンド `net_tester add --vport 1 --port 3 -S sockets` を実行
    Then 終了ステータスは 0 ではない
  ...
  Scenario: run せずに add するとエラー
    ...
    When I run `net_tester add --vport 1 --port 1 -S sockets`
